 ...
 * @param language 输出语言（可选，默认从 sessionStorage 获取）
 */
export interface FromDescriptionStreamCallbacks {
  onProgress: (data: { step: 'outline_parsed' | 'descriptions_split'; total: number }) => void;
  onDone: (data: { total: number; pages: Page[] }) => void;
  onError: (message: string) => void;
}

export const generateFromDescriptionStream = async (
  projectId: string,
  callbacks: FromDescriptionStreamCallbacks,
  ...
  const response = await fetch(`/api/projects/${projectId}/generate/from-description/stream`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(accessCode ? { 'X-Access-Code': accessCode } : {}),
